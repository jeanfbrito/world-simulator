...
fn initial_unit_spawn_system(mut commands: Commands, world_map: Res<crate::WorldMap>) {
    ...
    while spawned < 5 {
        ...
        if world_map.tiles[y][x].is_walkable() {
            ...
            println!(
                "{}",
                format!(
                    "[SPAWN] Peasant {} at ({}, {}) with consolidated components",
                    spawned, x, y
                )
                .green()
            );
    ...
    println!(
        "{}",
        "[SPAWN] Created 5 initial peasants with consolidated state components".green()
    );
}
