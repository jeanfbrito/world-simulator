...
impl ConstructionTask {
    ...
    pub fn update(&mut self, delta_time: f32) -> bool {
        ...
        if self.progress >= 1.0 {
            ...
            info!("[CONSTRUCTION] Completed {:?} at {:?}", self.building_type, self.position);
            return true;
        }
    ...
    pub fn assign_worker(&mut self) {
        ...
        if self.status == ConstructionStatus::Queued {
            ...
            info!("[CONSTRUCTION] Started {:?} at {:?}", self.building_type, self.position);
        }
    }
    ...
    pub fn cancel(&mut self) {
        ...
        info!("[CONSTRUCTION] Cancelled {:?} at {:?}", self.building_type, self.position);
    }
}
...
impl ConstructionQueue {
    ...
    pub fn add_task(&mut self, building_type: BuildingType, position: (i32, i32)) {
        ...
        info!("[CONSTRUCTION] Added {:?} to queue at position {:?}", 
    ...
    pub fn cancel_task(&mut self, position: (i32, i32)) -> bool {
        // Check active tasks
        if let Some(index) = self.active_tasks.iter().position(|t| t.position == position) {
