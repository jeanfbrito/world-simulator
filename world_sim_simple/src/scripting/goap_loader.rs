...
pub fn load_goap_actions_from_file(path: &Path) -> Result<Vec<GoapAction>, String> {
    ...
    let actions_table: LuaTable = lua
        .load(&script)
        .eval()
    ...
    for pair in actions_table.pairs::<LuaString, LuaTable>() {
        ...
        let cost: f32 = action_data.get("cost").unwrap_or(1.0);
        ...
        if let Ok(preconditions) = action_data.get::<LuaTable>("preconditions") {
            for pair in preconditions.pairs::<LuaString, LuaTable>() {
                let (key, value_data) =
                    pair.map_err(|e| format!("Failed to parse precondition: {}", e))?;
                let key_str = key
                    .to_str()
        ...
        if let Ok(effects) = action_data.get::<LuaTable>("effects") {
            for pair in effects.pairs::<LuaString, LuaTable>() {
                let (key, value_data) =
                    pair.map_err(|e| format!("Failed to parse effect: {}", e))?;
                let key_str = key
                    .to_str()
                    .map_err(|e| format!("Failed to convert key: {}", e))?
                    .to_string();
        ...
        match load_goap_actions_from_file(Path::new(&script_path)) {
            Ok(actions) => {
                ...
                debug.log(
                    ...
                    "GOAP_LOADER",
                    &format!(
                        "Successfully loaded {} GOAP actions from {}",
                        action_count, pack_name
                    ),
                );
            }
            Err(e) => {
...
pub fn merge_goap_actions_system(
    ...
    if let Some(scripted_actions) = scripted {
        ...
        debug.log(
            ...
            "GOAP_LOADER",
            &format!(
                "Merging {} scripted actions into ActionSet",
                scripted_actions.actions.len()
            ),
        );
        ...
        debug.log(
            ...
            "GOAP_LOADER",
            &format!(
                "ActionSet now contains {} actions",
