...
impl Default for SaveManager {
    fn default() -> Self {
        ...
        info!(
            "[SAVE] Save manager initialized with directory: {:?}",
            save_dir
        );
...
impl SaveManager {
    pub fn save_game(&self, save_state: &SaveState, filename: &str) -> Result<(), SaveError> {
        ...
        // Validate save state
        save_state.validate().map_err(SaveError::ValidationError)?;
    ...
    fn save_json(&self, save_state: &SaveState, path: &Path) -> Result<(), SaveError> {
        ...
        let mut file = fs::File::create(path).map_err(SaveError::IoError)?;
    ...
    fn load_json(&self, path: &Path) -> Result<SaveState, SaveError> {
        let mut file = fs::File::open(path).map_err(SaveError::IoError)?;
    ...
    fn load_binary(&self, path: &Path) -> Result<SaveState, SaveError> {
        ...
        file.read_to_end(&mut bytes).map_err(SaveError::IoError)?;
    ...
    pub fn list_saves(&self) -> Result<Vec<String>, SaveError> {
        let entries = fs::read_dir(&self.save_directory).map_err(SaveError::IoError)?;
    ...
    pub fn delete_save(&self, filename: &str) -> Result<(), SaveError> {
        let file_path = self.save_directory.join(filename);
        fs::remove_file(file_path).map_err(SaveError::IoError)?;
