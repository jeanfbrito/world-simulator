// Simplified Pack Loader
// Handles loading Lua files with immediate registration
...
impl PackSystem {
    ...
    /// Bind Lua API functions with immediate registration
    pub fn bind_lua_api(&mut self) -> Result<(), PackError> {
        // Use Arc<Mutex> for thread-safe access to registries
        let resource_registry = Arc::new(Mutex::new(&mut self.resource_registry as *mut ResourceRegistry));
        let item_registry = Arc::new(Mutex::new(&mut self.item_registry as *mut ItemRegistry));
        let recipe_registry = Arc::new(Mutex::new(&mut self.recipe_registry as *mut RecipeRegistry));
        let entity_registry = Arc::new(Mutex::new(&mut self.entity_registry as *mut EntityRegistry));

        let debug = self.metadata.config.debug;

        // Register resource function
        {
            let registry = resource_registry.clone();
            self.lua.globals().set("register_resource",
                self.lua.create_function(move |lua, table: LuaTable| {
                    ...
                    let registry_ptr = *registry.lock().unwrap();
                    unsafe {
                        (*registry_ptr).register(def.id.clone(), def)
                            .map_err(|e| LuaError::external(format!("Failed to register resource: {}", e)))?;
                    }
        ...
        {
            let registry = item_registry.clone();
            self.lua.globals().set("register_item",
                self.lua.create_function(move |lua, table: LuaTable| {
                    ...
                    unsafe {
                        (*registry_ptr).register(def.id.clone(), def)
                            .map_err(|e| LuaError::external(format!("Failed to register item: {}", e)))?;
                    }
        ...
        // Register recipe function
        {
            let registry = recipe_registry.clone();
            self.lua.globals().set("register_recipe",
                self.lua.create_function(move |lua, table: LuaTable| {
                    ...
                    unsafe {
                        (*registry_ptr).register(def.id.clone(), def)
                            .map_err(|e| LuaError::external(format!("Failed to register recipe: {}", e)))?;
                    }
        ...
        {
            let registry = entity_registry.clone();
            self.lua.globals().set("register_entity",
                self.lua.create_function(move |lua, table: LuaTable| {
                    ...
                    unsafe {
                        (*registry_ptr).register(def.id.clone(), def)
                            .map_err(|e| LuaError::external(format!("Failed to register entity: {}", e)))?;
                    }
