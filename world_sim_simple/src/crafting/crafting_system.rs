use super::{CraftingStationType, Recipe};
...
impl CraftingSystem {
    ...
    pub fn start_crafting(
        ...
        if let Some(required) = recipe.station_required {
            if station != required && station != CraftingStationType::None {
                return CraftingResult::Failed(format!("Recipe requires {}", required.name()));
            }
        }
    ...
    pub fn cancel_task(&mut self, recipe_id: &str) -> bool {
        ...
        if let Some(pos) = self
            .active_tasks
            .iter()
            .position(|t| t.recipe_id == recipe_id)
        {
