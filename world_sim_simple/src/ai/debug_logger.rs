...
use super::{WorldState, GoapAction, ActionPlan};
...
impl AIDebugLogger {
    ...
    pub fn log_agent_state(
        ...
        let agent_name = name.map(|n| n.name.clone()).unwrap_or_else(|| format!("Agent {:?}", entity));
        
        // Extract key state values
        let hunger = state.get("is_hungry").map(|v| format!("{:?}", v)).unwrap_or("N/A".to_string());
        let energy = state.get("has_energy").map(|v| format!("{:?}", v)).unwrap_or("N/A".to_string());
        let wood = state.get("has_wood").map(|v| format!("{:?}", v)).unwrap_or("0".to_string());
        let food = state.get("has_food").map(|v| format!("{:?}", v)).unwrap_or("0".to_string());
        let at_location = if state.get("at_resource").map(|v| matches!(v, super::StateValue::Bool(true))).unwrap_or(false) {
            "Resource"
        } else if state.get("at_storage").map(|v| matches!(v, super::StateValue::Bool(true))).unwrap_or(false) {
            "Storage"
        } else if state.get("at_home").map(|v| matches!(v, super::StateValue::Bool(true))).unwrap_or(false) {
            "Home"
        } else {
            "Wilderness"
        };
        
        println!("{} {} | Goal: {} | Hunger: {} | Energy: {} | Wood: {} | Food: {} | Location: {}",
            "👤".blue(),
            agent_name.cyan(),
            goal.yellow(),
            hunger.red(),
    ...
    pub fn log_planning_failure(
        ...
        println!("{} {} failed to plan for goal: {}",
            "❌".red(),
            agent_name.cyan(),
            goal.yellow()
        );
        ...
        println!("  {} Goal requirements not met",
            "ℹ".blue()
        );
    }
    ...
    pub fn log_plan_created(
        ...
        let action_names: Vec<&str> = plan.actions.iter()
            .map(|a| a.name.as_str())
            .collect();
        
        let total_cost: f32 = plan.actions.iter().map(|a| a.cost).sum();
        println!("{} {} created plan: [{}] (cost: {:.1})",
            "📋".green(),
            agent_name.cyan(),
    ...
    pub fn log_resource_summary(
        ...
        println!("{} Resources | Storage: {}🪵 {}⛏ {}🍖 | World: {}🌲 {}🪨 {}🫐",
            "📦".yellow(),
            wood_available,
            stone_available,
            food_available,
