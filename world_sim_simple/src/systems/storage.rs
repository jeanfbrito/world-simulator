        ...
        for (storage_entity, storage_pos, mut storage, stockpile, warehouse) in storages.iter_mut() {
            ...
            for (resource_type, amount) in inventory.get_all_resources() {
                ...
                if deposited_amount > 0 {
                    ...
                    println!("{} {} deposited {} {} into storage",
                        "📦".green(),
                        name.name.cyan(),
                        deposited_amount,
                    ...
                    debug.log(
                        DebugLevel::Info,
                        "STORAGE",
                        &format!("{} deposited {} {:?} into storage at ({},{})",
                            name.name, deposited_amount, resource_type,
...
pub fn storage_withdrawal_system(
    ...
    for (unit_entity, unit_pos, mut inventory, name) in units.iter_mut() {
        ...
        for (storage_entity, storage_pos, mut storage) in storages.iter_mut() {
            ...
            let available_resources: Vec<(ResourceType, u32)> = storage.stored.iter()
...
pub fn storage_task_assignment_system(
    ...
    for (unit_entity, unit_pos, inventory, mut task, work, name) in units.iter_mut() {
        ...
        for (storage_entity, storage_pos, storage) in storages.iter() {
            ...
            let can_store_any = inventory.get_all_resources()
                .iter()
        ...
        if let Some((target_entity, distance)) = best_storage {
            ...
            if let Some((resource_type, amount)) = inventory.get_all_resources()
                .iter()
                ...
                .map(|(t, a)| (*t, *a))
            {
                *task = StorageTask::new(target_entity, resource_type, amount);
                ...
                debug.log(
                    DebugLevel::Info,
                    "STORAGE",
                    &format!("{} assigned to deposit {} {:?} to storage ({}m away)",
...
pub fn spawn_storage_buildings_system(
    ...
    commands.spawn((
        ...
        GridPosition { x: center_x, y: center_y },
        TileEntity { x: center_x as usize, y: center_y as usize },
        StorageBuilding::new(900), // 3x3 * 100 capacity
        Stockpile::new(3, 3),
        StorageUpdateTag,
    ));
    
    println!("{} Central Stockpile created at ({}, {})",
        "🏗️".green(),
        center_x,
        center_y
    );
    ...
    commands.spawn((
        ...
        GridPosition { x: center_x + 5, y: center_y },
        TileEntity { x: (center_x + 5) as usize, y: center_y as usize },
        StorageBuilding::new(2000), // Larger capacity
        Warehouse::new(),
        StorageUpdateTag,
    ));
    
    println!("{} Main Warehouse created at ({}, {})",
        "🏢".cyan(),
        center_x + 5,
        center_y
    );
...
pub fn warehouse_efficiency_system(
    ...
    for (mut storage, warehouse) in warehouses.iter_mut() {
        ...
        debug.log(
            DebugLevel::Debug,
            "STORAGE",
            &format!("Warehouse efficiency: {:.0}%, capacity: {}",
                efficiency * 100.0, storage.total_capacity)
        );
    }
}
...
pub fn storage_display_system(
    ...
    for (name, storage) in storages.iter() {
        ...
        debug.log(
            DebugLevel::Info,
            "STORAGE",
            &format!("{} contains: {} ({:.0}% full)",
                name.name, contents, storage.fill_percentage() * 100.0)
        );
    }
}
...
pub fn storage_performance_monitor_system(
    ...
    debug.log(
        DebugLevel::Debug,
        "PERFORMANCE",
        &format!("Storage: {} buildings, {}/{} capacity ({:.0}% util), {} active tasks",
