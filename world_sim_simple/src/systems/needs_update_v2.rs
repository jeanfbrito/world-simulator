...
pub fn sync_needs_v2_to_worldstate_system(
    ...
    for (needs, inventory, location, mut world_state) in query.iter_mut() {
        ...
        world_state.set(
            "is_hungry",
            crate::ai::StateValue::Float(needs.hunger() as f64),
        );
        world_state.set(
            "has_energy",
            crate::ai::StateValue::Float(needs.energy() as f64),
        );
        world_state.set(
            "morale",
            crate::ai::StateValue::Float(needs.morale() as f64),
        );
        world_state.set(
            "has_shelter",
            crate::ai::StateValue::Bool(needs.has_shelter),
        );
        ...
        world_state.set(
            "critical_hunger",
            crate::ai::StateValue::Bool(needs.is_starving()),
        );
        world_state.set(
            "critical_energy",
            crate::ai::StateValue::Bool(needs.is_exhausted()),
        );
        ...
        world_state.set(
            "inventory_full",
            crate::ai::StateValue::Bool(inventory.is_full()),
        );

        // Sync location (unchanged)
        world_state.set(
            "at_storage",
            crate::ai::StateValue::Bool(location.is_at_storage()),
        );
        world_state.set(
            "at_home",
            crate::ai::StateValue::Bool(location.is_at_home()),
        );
        world_state.set(
            "at_resource",
            crate::ai::StateValue::Bool(location.is_at_resource()),
        );
    }
}
